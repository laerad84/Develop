...
int main(int argc, char** argv){
  ...
  */
    ...
    for( int grIndex = 0; grIndex < 16; grIndex++){
      //grTempLong[grIndex] = *(TGraph*)mapDr230->Get(Form("TEMPLONG%02d",grIndex),grTempLong[grIndex]);
      *grTempLong[grIndex] = *((TGraph*)mapDr230->Get(Form("TEMPLONG%02d",grIndex),grTempLong[grIndex]));
    }
    for( int grIndex = 0; grIndex < 3; grIndex++){
      //grPresLong[grIndex] = (TGraph*)mapDr230->Get(Form("PRESLONG%02d",grIndex),grPresLong[grIndex]);
      *grPresLong[grIndex] = *((TGraph*)mapDr230->Get(Form("PRESLONG%02d",grIndex),grPresLong[grIndex]));
    }
    for( int i = 0; i< 60; i++){
      grLong[i]    = grTempKEY[i];
