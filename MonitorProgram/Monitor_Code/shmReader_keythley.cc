...
bool init_shm(int &id, key_t &key, char* keyFile, struct KEYTHLEYdata **data){
  ...
  if(( *data = (struct KEYTHLEYdata *)shmat(id,0,0)) == (void *)-1) {
    perror("shmat");
    //return false;
    exit(1);
  }
...
int main(int argc , char** argv)
{
  ...
  struct KEYTHLEYdata *temperature = new KEYTHLEYdata;
  ...
  while(1){
    std::cout  << temperature->now << " : "  << temperature->data << std::endl;
    usleep(1000000);
  }
